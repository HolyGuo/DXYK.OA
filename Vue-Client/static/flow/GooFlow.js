...
var GooFlow = function(selector, property) {
  ...
  this.$workArea.on('click', {
    ...
    This.addNode(new Date().getTime().toString(), {
      name: 'node_' + This.$max,
      left: X,
      top: Y,
      type: This.$nowType
...
GooFlow.prototype = {
  ...
  _initDraw: function(id, width, height) {
    ...
    // 绑定右键事件
    $(this.$draw).on('contextmenu', tmpClk, {
      inthis: this
    }, function(e) {
      var This = e.data.inthis
  ...
  _initGroup: function(width, height) {
    ...
    // 绑定右键事件
    this.$group.on('contextmenu', '.GooFlow_area', {
      inthis: this
    }, function(e) {
      var This = e.data.inthis
    ...
    this.$group.on('mousedown', {
      ...
      var ev = _mouseP(e),
        t = This.t // t=_elCsys(This.$workArea[0]);
  ...
  _initWorkForNode: function() {
    ...
    // 绑定双击(包括双击编辑)事件
    this.$workArea.on('dblclick', '.GooFlow_item > .span', {
      inthis: this
    }, function(e) {
    ...
    this.$workArea.on('dblclick', '.ico + td', {
      inthis: this
    }, function(e) {
      var id = $(this).parents('.GooFlow_item').attr('id')
      var This = e.data.inthis
    ...
    // 以下是工作区为编辑模式时才绑定的事件
    // 绑定用鼠标移动事件
    this.$workArea.on('mousedown', '.ico', {
      inthis: this
    }, function(e) {
      if (!e) e = window.event
      if (e.button === 2) return false
    ...
    // 绑定鼠标覆盖/移出事件
    this.$workArea.on('mouseenter', '.GooFlow_item', {
      inthis: this
    }, function(e) {
    ...
    this.$workArea.on('mouseleave', '.GooFlow_item', {
      inthis: this
    }, function(e) {
      if ((e.data.inthis.$nowType !== 'direct' && e.data.inthis.$nowType !== 'dashed') && !document.getElementById('GooFlow_tmp_line')) return
    ...
    // 绑定结点的删除功能
    this.$workArea.on('click', '.rs_close', {
      inthis: this
    }, function(e) {
      if (!e) e = window.event
    ...
    // 绑定结点的RESIZE功能
    this.$workArea.on('mousedown', '.GooFlow_item > div > div[class!=rs_close]', {
      inthis: this
    }, function(e) {
      if (!e) e = window.event
      if (e.button === 2) return false
  ...
  addNode: function(id, json) {
    ...
    if (GooFlow.color.node) {
      if (json.type.indexOf(' mix') > -1) {
        this.$nodeDom[id].css({
          'background-color': GooFlow.color.mix,
          'border-color': GooFlow.color.mix
        })
        ...
        this.$nodeDom[id].css({
          'background-color': GooFlow.color.node,
          'border-color': GooFlow.color.node
        })
      }
      if (mark && GooFlow.color.mark) {
        this.$nodeDom[id].css({
          'border-color': GooFlow.color.mark
        })
      }
    }
    if (json.type.indexOf(' mix') > -1) {
  ...
  setLine(id, obj) {
    Object.assign(this.$lineData[id], obj)
  },
  // 设置结点/连线/分组区域的文字信息
  ...
  addLine: function(id, json) {
    ...
    if (json.Compares) this.$lineData[id].Compares = json.Compares
