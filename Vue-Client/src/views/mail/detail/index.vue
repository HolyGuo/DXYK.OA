...
export default {
  ...
  methods: {
    initPage() {
      this.mailId = this.$store.getters.mailId;
    ...
    getDetail() {
      ...
      mailAPI.fetchMailDetail(this.mailId).then(res => {
        this.loading = false;
        this.mail = res.result;
      });
      mailAPI.fetchReceiver(this.mailId).then(res => {
