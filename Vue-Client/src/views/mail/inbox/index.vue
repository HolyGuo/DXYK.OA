<template>
    <div class="app-container calendar-list-container">
        ...
        <div class="filter-container">
            ...
            <el-button @click="forward" type="primary" icon="el-icon-share" class="tool-item filter-item btn-forward">
            </el-button>
            <el-button type="danger" icon="el-icon-delete" class="tool-item filter-item btn-del" v-on:click="handleDelete()">
            </el-button>
        ...
        <el-table :key='tableKey' :data="list" ref="multipleTable" @sort-change="customSort" @selection-change="handleSelectionChange" v-loading.body="listLoading" border highlight-current-row style="width: 100%">
            ...
            <el-table-column prop="readDate" sortable="custom" align="center" label="阅读时间" width="150px">
                <template scope="scope">
                    <span>{{scope.row.read_time | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
                </template>
            </el-table-column>
        </el-table>
    ...
    methods: {
        ...
        handleDelete() {
            ...
            this.$confirm('是否删除这' + selectedLen + '封邮件?', '提示', {
                ...
                mailAPI.delReceiveMail(idArr).subscribe({
                    next: () => {
                        this.$message({
                            message: '删除成功',
                            type: 'success',
