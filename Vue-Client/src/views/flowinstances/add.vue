    ...
    <div class="createPost-container">
      <el-form class="form-container" :model="postObj" :rules="rules" ref="postObj">
        ...
        <div class="createPost-main-container" v-show="active==0">
          <el-form-item label=" ">
            <el-select v-model="currentSchemeId" size="small" placeholder="请选择">
          ...
          <div class="flow-form-content">
            <flow-editor ref="flowEditor" :scheme-content="currentScheme.schemeContent"></flow-editor>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      onSchemeChange(schemeId) {
        // 预览表单和流程
        var _this = this
        this.currentScheme = this.flowschemes.find(u => u.id === schemeId)
        ...
        _this.$refs.postObj.validate(valid => {
          if (valid) {
            ...
            if (_this.postObj.frmType === 1) { // 使用的是自定义的页面提交
              ...
              console.log(_this.$refs.frmData)
              var iframes = _this.$refs.frmData.querySelectorAll('iframe')
              const frmdata = {}
              iframes.forEach((item) => {
                var data = item.contentWindow.getData()
                frmdata[item.name] = data
              })
              console.log(frmdata)
              _this.loading = false
              _this.$notify({
